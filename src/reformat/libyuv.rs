...
pub fn yuv_to_rgb(image: &image::Image, rgb: &mut rgb::Image) -> AvifResult<bool> {
    ...
    let mut plane_u8: [*const u8; 4] = ALL_PLANES
        ...
        .map(|x| {
            if image.has_plane(*x) {
                image.planes[x.to_usize()].as_ref().unwrap().ptr()
            } else {
                std::ptr::null()
            }
        })
    ...
    let plane_u16: [*const u16; 4] = ALL_PLANES
        ...
        .map(|x| {
            if image.has_plane(*x) {
                image.planes[x.to_usize()].as_ref().unwrap().ptr16()
            } else {
                std::ptr::null()
            }
        })
            ...
            plane_u8 = ALL_PLANES
                ...
                .map(|x| {
                    if image8.has_plane(*x) {
                        image8.planes[x.to_usize()].as_ref().unwrap().ptr()
                    } else {
                        std::ptr::null()
                    }
                })
...
fn downshift_to_8bit(
    ...
    for plane in ALL_PLANES {
        ...
        let source_ptr = image.planes[plane.to_usize()].as_ref().unwrap().ptr16();
        let pd8 = image8.plane_data(plane).unwrap();
        let dst_ptr = image8.planes[plane.to_usize()].as_mut().unwrap().ptr_mut();
        unsafe {
            Convert16To8Plane(
                source_ptr,
