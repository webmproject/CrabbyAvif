...
impl Encoder {
    ...
    pub(crate) fn write_jpegxl_ftyp(&self, stream: &mut OStream) -> AvifResult<()> {
        ...
        let (major_brand, compatible_brands): (String, &[std::string::String]) =
            if self.is_sequence() {
                (
                    String::from("hxlS"),
                    &[
                ...
                (
                    String::from("hxlI"),
                    &[String::from("mif1"), String::from("miaf")],
                )
            };
        // unsigned int(32) major_brand;
        ...
        for item in items {
            ...
            if item.extra_layer_count > 0 {
                ...
                for i in 0..layer_count as usize {
                    ...
                    stream.write_u32(u32_from_usize(item.samples[i].sample_data().len())?)?;
                }
            } else {
                // unsigned int(16) extent_count;
                stream.write_u16(1)?;
                ...
                let extent_length = if item.samples.is_empty() {
                    ...
                    u32_from_usize(item.samples[0].sample_data().len())?
                };
                // unsigned int(length_size*8) extent_length;
                ...
                if !item.samples.is_empty() {
                    if item.samples.len() > 1 {
                        ...
                        for sample in &item.samples {
                            stream.write_slice(sample.sample_data())?;
                        }
                    } else {
                        chunk_offset = stream
                            .write_slice_dedupe(mdat_start_offset, item.samples[0].sample_data())?;
                    }
        ...
        for layered_item_id in &layered_item_ids {
            ...
            loop {
                ...
                for item_id in layered_item_id {
                    ...
                    stream.write_slice(item.samples[layer_index].sample_data())?;
                    stream.write_u32_at_offset(
                        u32_from_usize(chunk_offset)?,
