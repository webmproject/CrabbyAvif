...
impl Encoder {
    ...
    fn write_mini(&self, stream: &mut OStream) -> AvifResult<()> {
        ...
        let mut large_dimensions_flag = image.width - 1 >= (1 << 7) || image.height - 1 >= (1 << 7);
        ...
        let mut large_item_data_flag = color_data.len() - 1 >= (1 << 15)
            || (alpha_data.is_some() && alpha_data.unwrap().len() >= (1 << 15));
        let mut large_metadata_flag = (has_icc && image.icc.len() - 1 >= (1 << 10))
            || (image.exif.len() != 0 && image.exif.len() - 1 >= (1 << 10))
            || (image.xmp.len() != 0 && image.xmp.len() - 1 >= (1 << 10));
        ...
        if has_gainmap {
            ...
            large_dimensions_flag = large_dimensions_flag
                || gainmap_image.width - 1 >= (1 << 7)
                || gainmap_image.height - 1 >= (1 << 7);
            ...
            large_metadata_flag = large_metadata_flag
                || (!self.alt_image_metadata.icc.is_empty()
                    && self.alt_image_metadata.icc.len() - 1 >= (1 << 10))
        ...
        if has_hdr && has_gainmap && gainmap_data.unwrap().len() != 0 {
        ...
        if has_alpha && alpha_data.unwrap().len() != 0 {
        ...
        if codec_config_size > 0 {
            if let CodecConfiguration::Av1(config) = &color_item.codec_configuration {
                ...
                return Err(AvifError::UnknownError(
                    "Unexpected codec configuration".into(),
                ));
            }
        }
        if has_alpha && alpha_data.unwrap().len() != 0 && alpha_codec_config_size != 0 {
...
fn image_irot_imir_to_exif_orientation(image: &Image) -> u8 {
    if image.irot_angle.is_none() || image.irot_angle == Some(0) {
        if image.imir_axis.is_none() {
            return 1; // The 0th row is at the visual top of the image, and the 0th column is the visual left-hand side.
        }
        if image.imir_axis == Some(0) {
