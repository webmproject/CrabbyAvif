...
pub(crate) fn parse_mini(stream: &mut IStream, offset: usize) -> AvifResult<MetaBox> {
    let mut stream = stream.sub_bit_stream(stream.bytes_left()?)?;
    let mut meta = MetaBox::default();

    let version = stream.read(2)?; // bit(2) version = 0;
    if version != 0 {
    ...
    let chroma_subsampling = stream.read(2)?; // bit(2) chroma_subsampling;
    let orientation = stream.read(3)? + 1; // bit(3) orientation_minus1;

    // Spatial extents
    let large_dimensions_flag = stream.read_bool()?; // bit(1) large_dimensions_flag;
    let width = stream.read(if large_dimensions_flag { 15 } else { 7 })? + 1; // unsigned int(large_dimensions_flag ? 15 : 7) width_minus1;
    let height = stream.read(if large_dimensions_flag { 15 } else { 7 })? + 1; // unsigned int(large_dimensions_flag ? 15 : 7) height_minus1;
    ...
    let bit_depth = if float_flag {
        ...
        return Err(AvifError::NotImplemented); // Either invalid AVIF or unsupported non-AVIF.
    } else {
        let high_bit_depth_flag = stream.read_bool()?; // bit(1) high_bit_depth_flag;
        if high_bit_depth_flag {
            stream.read(3)? + 9 // bit(3) bit_depth_minus9;
        } else {
            8
        }
    };
    ...
    if has_explicit_cicp {
        color_primaries = ColorPrimaries::from(stream.read(8)? as u16); // bit(8) colour_primaries;
        transfer_characteristics = TransferCharacteristics::from(stream.read(8)? as u16); // bit(8) transfer_characteristics;
        matrix_coefficients = if chroma_subsampling != 0 {
            MatrixCoefficients::from(stream.read(8)? as u16) // bit(8) matrix_coefficients;
        } else {
    ...
    if has_hdr {
        ...
        if has_gainmap {
            gainmap_width = stream.read(if large_dimensions_flag { 15 } else { 7 })? + 1; // unsigned int(large_dimensions_flag ? 15 : 7) gainmap_width_minus1;
            gainmap_height = stream.read(if large_dimensions_flag { 15 } else { 7 })? + 1; // unsigned int(large_dimensions_flag ? 15 : 7) gainmap_height_minus1;
            gainmap_matrix_coefficients = MatrixCoefficients::from(stream.read(8)? as u16); // bit(8) gainmap_matrix_coefficients;
            gainmap_full_range = stream.read_bool()?; // bit(1) gainmap_full_range_flag;

            gainmap_chroma_subsampling = stream.read(2)?; // bit(2) gainmap_chroma_subsampling;
            ...
            gainmap_bit_depth = if gainmap_float_flag {
                ...
                return Err(AvifError::BmffParseFailed(
                    "gainmap_float_flag cannot be 1 for AV1".into(),
                ...
                if gainmap_high_bit_depth_flag {
                    stream.read(3)? + 9 // bit(3) gainmap_bit_depth_minus9;
                } else {
                    8
                }
            };
            ...
            if tmap_has_explicit_cicp {
                tmap_color_primaries = ColorPrimaries::from(stream.read(8)? as u16); // bit(8) tmap_colour_primaries;
                tmap_transfer_characteristics =
                    TransferCharacteristics::from(stream.read(8)? as u16); // bit(8) tmap_transfer_characteristics;
                tmap_matrix_coefficients = MatrixCoefficients::from(stream.read(8)? as u16); // bit(8) tmap_matrix_coefficients;
        ...
        clli = parse_mini_hdrproperties(&mut stream)?;
        if has_gainmap {
            tmap_clli = parse_mini_hdrproperties(&mut stream)?;
        }
    }
    ...
    let icc_data_size = if has_icc {
        stream.read(if large_metadata_flag { 20 } else { 10 })? + 1 // unsigned int(large_metadata_flag ? 20 : 10) icc_data_size_minus1;
    } else {
        0
    };
    let tmap_icc_data_size = if has_hdr && has_gainmap && tmap_has_icc {
        stream.read(if large_metadata_flag { 20 } else { 10 })? + 1 // unsigned int(large_metadata_flag ? 20 : 10) tmap_icc_data_size_minus1;
    } else {
        0
    };

    let gainmap_metadata_size = if has_hdr && has_gainmap {
        stream.read(if large_metadata_flag { 20 } else { 10 })? // unsigned int(large_metadata_flag ? 20 : 10) gainmap_metadata_size;
    } else {
        0
    };
    let gainmap_item_data_size = if has_hdr && has_gainmap {
        stream.read(if large_item_data_flag { 28 } else { 15 })? // unsigned int(large_item_data_flag ? 28 : 15) gainmap_item_data_size;
    } else {
        0
    };
    let mut gainmap_item_codec_config_size = if has_hdr && has_gainmap && gainmap_item_data_size > 0
    {
        stream.read(if large_codec_config_flag { 12 } else { 3 })? // unsigned int(large_codec_config_flag ? 12 : 3) gainmap_item_codec_config_size;
    } else {
        0
    };

    let main_item_codec_config_size = stream.read(if large_codec_config_flag { 12 } else { 3 })?; // unsigned int(large_codec_config_flag ? 12 : 3) main_item_codec_config_size;
    let main_item_data_size = stream.read(if large_item_data_flag { 28 } else { 15 })? + 1; // unsigned int(large_item_data_flag ? 28 : 15) main_item_data_size_minus1;

    let alpha_item_data_size = if has_alpha {
        stream.read(if large_item_data_flag { 28 } else { 15 })? // unsigned int(large_item_data_flag ? 28 : 15) alpha_item_data_size;
    } else {
        0
    };
    let mut alpha_item_codec_config_size = if has_alpha && alpha_item_data_size != 0 {
        stream.read(if large_codec_config_flag { 12 } else { 3 })? // unsigned int(large_codec_config_flag ? 12 : 3) alpha_item_codec_config_size;
    } else {
        0
    };
    ...
    let exif_data_size = if has_exif {
        stream.read(if large_metadata_flag { 20 } else { 10 })? + 1 // unsigned int(large_metadata_flag ? 20 : 10) exif_data_size_minus_one;
    } else {
        0
    };
    let xmp_data_size = if has_xmp {
        stream.read(if large_metadata_flag { 20 } else { 10 })? + 1 // unsigned int(large_metadata_flag ? 20 : 10) xmp_data_size_minus_one;
    } else {
        0
    };
    ...
    if main_item_codec_config_size != 4 {
        return Err(AvifError::BmffParseFailed(
            "av1C should be 4-byte long".into(),
        ));
    }
    let main_item_codec_config = parse_av1_codec_configuration(&mut stream.sub_stream(4 * 8)?)?; // unsigned int(8) main_item_codec_config[main_item_codec_config_size];
    let alpha_item_codec_config = if has_alpha && alpha_item_data_size != 0 {
        Some(if alpha_item_codec_config_size == 0 {
            ...
            if alpha_item_codec_config_size != 4 {
                return Err(AvifError::BmffParseFailed(
                    "av1C should be 4-byte long".into(),
                ));
            }
            parse_av1_codec_configuration(&mut stream.sub_stream(4 * 8)?)? // unsigned int(8) alpha_item_codec_config[alpha_item_codec_config_size];
        })
    } else {
        None
    };
    let gainmap_item_codec_config = if has_hdr && has_gainmap {
        Some(if gainmap_item_codec_config_size == 0 {
            ...
            if gainmap_item_codec_config_size != 4 {
                return Err(AvifError::BmffParseFailed(
                    "av1C should be 4-byte long".into(),
                ));
            }
            parse_av1_codec_configuration(&mut stream.sub_stream(4 * 8)?)? // unsigned int(8) gainmap_item_codec_config[gainmap_item_codec_config_size];
        })
    } else {
        None
    };
    ...
    assert_eq!(stream.bit_offset % 8, 0);
    let offset_till_remaining_bytes = stream.bit_offset / 8;
    ...
    if remaining_bytes.len() as u32
        != icc_data_size
            ...
            + xmp_data_size
    {
        return Err(AvifError::BmffParseFailed("Unexpected mini size".into()));
    }
...
fn skip_mastering_display_colour_volume(stream: &mut IBitStream) -> Result<(), AvifError> {
    for _ in [0, 1, 2] {
        stream.skip(16)?; // unsigned int(16) display_primaries_x;
        stream.skip(16)?; // unsigned int(16) display_primaries_y;
    }
    stream.skip(16)?; // unsigned int(16) white_point_x;
    stream.skip(16)?; // unsigned int(16) white_point_y;
    stream.skip(32)?; // unsigned int(32) max_display_mastering_luminance;
    stream.skip(32)?; // unsigned int(32) min_display_mastering_luminance;
    Ok(())
}

fn skip_content_colour_volume(stream: &mut IBitStream) -> Result<(), AvifError> {
    stream.skip(1)?; // unsigned int(1) reserved = 0; // ccv_cancel_flag
    stream.skip(1)?; // unsigned int(1) reserved = 0; // ccv_persistence_flag
    ...
    stream.skip(2)?; // unsigned int(2) reserved = 0;

    if ccv_primaries_present {
        for _ in [0, 1, 2] {
            stream.skip(32)?; // signed int(32) ccv_primaries_x[[c]];
            stream.skip(32)?; // signed int(32) ccv_primaries_y[[c]];
        }
    }
    if ccv_min_luminance_value_present {
        stream.skip(32)?; // unsigned int(32) ccv_min_luminance_value;
    }
    if ccv_max_luminance_value_present {
        stream.skip(32)?; // unsigned int(32) ccv_max_luminance_value;
    }
    if ccv_avg_luminance_value_present {
        stream.skip(32)?; // unsigned int(32) ccv_avg_luminance_value;
    }
    Ok(())
}

fn skip_ambient_viewing_environment(stream: &mut IBitStream) -> Result<(), AvifError> {
    stream.skip(32)?; // unsigned int(32) ambient_illuminance;
    stream.skip(16)?; // unsigned int(16) ambient_light_x;
    stream.skip(16)?; // unsigned int(16) ambient_light_y;
    Ok(())
}

fn skip_reference_viewing_environment(stream: &mut IBitStream) -> Result<(), AvifError> {
    stream.skip(32)?; // unsigned int(32) surround_luminance;
    stream.skip(16)?; // unsigned int(16) surround_light_x;
    stream.skip(16)?; // unsigned int(16) surround_light_y;
    stream.skip(32)?; // unsigned int(32) periphery_luminance;
    stream.skip(16)?; // unsigned int(16) periphery_light_x;
    stream.skip(16)?; // unsigned int(16) periphery_light_y;
    Ok(())
}

fn skip_nominal_diffuse_white(stream: &mut IBitStream) -> Result<(), AvifError> {
    stream.skip(32)?; // unsigned int(32) diffuse_white_luminance;
    Ok(())
}

fn parse_mini_hdrproperties(
    stream: &mut IBitStream,
...
fn pixel_format_and_centered_to_chroma_sample_position(
    ...
    match (
        ...
        (PixelFormat::Yuv422, true, _) => Err(AvifError::BmffParseFailed(
            "chroma_is_horizontally_centered should be false".into(),
        )),
        ...
        (
            ...
            _,
            _,
        ) => Err(AvifError::BmffParseFailed(format!(
            "Unexpected pixel format {pixel_format:?}"
        ))),
    }
}
...
fn check_subsampling(
    ...
    {
        return Err(AvifError::BmffParseFailed(
            "Mismatch between mini and AV1 codec config".into(),
        ));
    }
    Ok(())
}
