...
fn add_avm_pkt_to_output_samples(
    ...
    //                    https://aomediacodec.github.io/av2-isobmff/pr/40/index.html#sync-sample
    ...
    output_samples.push(Sample::create_from(encoded_data, sample_data_range, sync)?);
    Ok(true)
}
...
impl Encoder for Avm {
    ...
    fn get_codec_config(
        ...
        let config_obus_range =
            Av2CodecConfiguration::get_config_obus_range(&output_samples[0].data_output_by_codec)?;
        let config_obus =
            &output_samples[0].data_output_by_codec[config_obus_range.0..config_obus_range.1];
        Ok(CodecConfiguration::Av2(
            Av2CodecConfiguration::from_av2_config_obus(config_obus)?,
        ))
    }
}
            ...
            if !next_frame.is_null() {
                ...
                let av2_payload = if first_time {
                    ...
                    config_obus_then_av2_payload
                        ...
                        .map_err(AvifError::map_out_of_memory)?;
                    config_obus_then_av2_payload.extend_from_slice(config_obus);
                    config_obus_then_av2_payload.extend_from_slice(av2_payload);
                    &config_obus_then_av2_payload
                } else {
                    av2_payload
                };
...
fn avm_enc_dec_test() -> Result<(), AvifError> {
    ...
    let config_obus =
        &output_samples[0].data_output_by_codec[config_obus_range.0..config_obus_range.1];
    let codec_config = Av2CodecConfiguration::from_av2_config_obus(config_obus)?;
