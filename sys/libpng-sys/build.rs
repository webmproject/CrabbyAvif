...
fn main() -> Result<(), String> {
    ...
    if Path::new(&library_file).exists() {
        ...
        match pkg_config::Config::new().probe("libpng") {
            ...
            Err(_) => {
                return Err("libpng could not be found locally or with pkg-config. \
                    Disable the png feature, install the libpng-dev system library, \
                    or build the dependency locally by running zlibpng.cmd from sys/libpng-sys."
                    .into());
            }
        }
    }
    ...
    let outfile = PathBuf::from(&outdir).join("libpng_bindgen.rs");
    ...
    let bindings = bindings
        .generate()
        .unwrap_or_else(|_| panic!("Unable to generate bindings for libpng."));
    bindings
        .write_to_file(outfile.as_path())
        .unwrap_or_else(|_| panic!("Couldn't write bindings for libpng"));
    Ok(())
}
