...
// Build rust library and bindings for libjpeg.
...
fn main() -> Result<(), String> {
    ...
    let abs_library_dir = PathBuf::from(&project_root).join("libjpeg-turbo");
    ...
    if Path::new(&library_file).exists() {
        ...
        match pkg_config::Config::new().probe("libjpeg") {
            Ok(library) => {
            ...
            Err(_) => {
                return Err(
                    "libjpeg-turbo could not be found locally or with pkg-config. \
                    Disable the jpeg feature, install the libjpeg-dev system library, \
                    or build the dependency locally by running libjpeg.cmd from sys/libjpeg-sys."
                        .into(),
                );
            }
        }
    ...
    let outfile = PathBuf::from(&outdir).join("libjpeg_bindgen.rs");
    ...
    let bindings = bindings
        .generate()
        .unwrap_or_else(|_| panic!("Unable to generate bindings for libjpeg."));
    bindings
        .write_to_file(outfile.as_path())
        .unwrap_or_else(|_| panic!("Couldn't write bindings for libjpeg"));
    Ok(())
}
